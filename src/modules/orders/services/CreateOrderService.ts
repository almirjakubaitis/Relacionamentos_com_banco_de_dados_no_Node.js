...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (checkExistentProducts.length) {
      throw new AppError(
        `Could not find a product in the list: ${checkExistentProducts[0].id}`,
      );
    }
    ...
    if (findProductsWithNoQuantityAvailable.length) {
      throw new AppError(
        `The quantity ${findProductsWithNoQuantityAvailable[0].quantity} is no available for the product: ${findProductsWithNoQuantityAvailable[0].id}`,
      );
    }
